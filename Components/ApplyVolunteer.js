...
const ApplyVolunteer = () => {
    ...
    const uploadFile = async (file) => {
        ...
        try {
            ...
            const response = await axios.post("https://api.cloudinary.com/v1_1/do7syekzp/image/upload", data);
    ...
    const setData = async (e) => {
        ...
        if (new Date().getFullYear() - new Date(birthday).getFullYear() !== parseInt(age)) {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:3003/api/apply", formData);
            console.log(response);
            setSuccessMessage("Volunteer application submitted successfully.");
            setIsPending(false);
            navigate(`/profile/${nid}`); // Navigate to the profile page with volunteer's nid
        } catch (error) {
            console.error(error);
            setIsPending(false);
    ...
    return (
        <div className="addForm">
            ...
            <form onSubmit={setData}>
                ...
                NationalId:<br />
                <input type="text" id="nid" name="nid" placeholder="Enter your national id"
                onChange = {(e) => {
                ...
                Contact Number:<br />
                <input type="text" id="contact" name="contact" required pattern="[0-9]{10}" title="Please enter a 10-digit number" 
                onChange = {(e) => {
                    setContact(e.target.value)
                }}/>
                <span className="error" id="errorMessage"></span><br /><br />
                ...
                <label htmlFor="child_education">Child education</label><br />

                <input type="radio" id="psychological_counselling" name="interest" value="psychological_counselling" 
                onChange = {(e) => {
                    setInterest(e.target.value)
                }}/>
                <label htmlFor="psychological_counselling">Psychological counselling</label><br />

                <input type="radio" id="human_rights_counselling" name="interest" value="human_rights_counselling" 
                onChange = {(e) => {
                    setInterest(e.target.value)
                }}/>
                <label htmlFor="human_rights_counselling">Human rights counselling</label><br />

                <input type="radio" id="legal_counselling" name="interest" value="legal_counselling"
                onChange = {(e) => {
                    setInterest(e.target.value)
                }} />
                <label htmlFor="legal_counselling">Legal counselling</label><br />
